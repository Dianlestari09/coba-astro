import { s as supabase, c as checkConnection } from './db_DqdQ79mS.mjs';
...
const FALLBACK_BLOGS = [
  {
    ...
    data: {
      ...
      pubDate: /* @__PURE__ */ new Date("2022-07-08"),
...
async function getDbBlogs() {
  try {
    ...
    const { data: rows, error } = await supabase.from("blogs").select("*").order("pubDate", { ascending: false });
    ...
    return rows.map((row) => ({
      ...
      data: {
        ...
        updatedDate: row.updatedDate ? new Date(row.updatedDate) : void 0,
...
async function getDbBlogBySlug(slug) {
  try {
    if (!await isConnected()) {
      return FALLBACK_BLOGS.find((b) => b.id === slug) || null;
    }
    const { data: rows, error } = await supabase.from("blogs").select("*").eq("slug", slug);
    if (error) throw error;
...
async function getDbBlogsExcept(slug, limit = 3) {
  try {
    if (!await isConnected()) {
      return FALLBACK_BLOGS.filter((b) => b.id !== slug).slice(0, limit);
    }
    const { data: rows, error } = await supabase.from("blogs").select("*").neq("slug", slug).order("pubDate", { ascending: false }).limit(limit);
    if (error) throw error;
...
async function saveDbBlog(data) {
  ...
  if (data.id) {
    ...
    payload.pubDate = data.pubDate || (/* @__PURE__ */ new Date()).toISOString();
...
async function getRawBlogsTable() {
  if (!await isConnected()) return [];
  const { data, error } = await supabase.from("blogs").select("*").order("pubDate", { ascending: false });
  if (error) throw error;
  return data || [];
}
async function getRawBlogById(id) {
  if (!await isConnected()) return null;
  const { data: rows, error } = await supabase.from("blogs").select("*").eq("id", id);
  if (error) throw error;
...
async function getDbServices() {
  try {
    ...
    const { data: rows, error } = await supabase.from("services").select("*").order("pubDate", { ascending: false });
    if (error) throw error;
...
async function getDbServiceBySlug(slug) {
  try {
    if (!await isConnected()) {
      return FALLBACK_SERVICES.find((s) => s.id === slug) || null;
    }
    const { data: rows, error } = await supabase.from("services").select("*").eq("slug", slug);
    if (error) throw error;
    if (!rows || rows.length === 0) return null;
    const row = rows[0];
    return {
...
async function saveDbService(data) {
  const conn = await isConnected();
  if (!conn) throw new Error("Database not connected. Cannot perform write operations.");
  const payload = {
    slug: data.slug,
...
async function deleteDbService(id) {
  const conn = await isConnected();
  if (!conn) throw new Error("Database not connected. Cannot perform write operations.");
  const { error } = await supabase.from("services").delete().eq("id", id);
  if (error) throw error;
}
async function getRawServicesTable() {
  if (!await isConnected()) return [];
  const { data, error } = await supabase.from("services").select("*").order("pubDate", { ascending: false });
  if (error) throw error;
  return data || [];
}
async function getRawServiceById(id) {
  if (!await isConnected()) return null;
  const { data: rows, error } = await supabase.from("services").select("*").eq("id", id);
  if (error) throw error;
  return rows && rows.length > 0 ? rows[0] : null;
}
async function getDbTeams() {
  try {
    ...
    const { data: rows, error } = await supabase.from("teams").select("*").order("pubDate", { ascending: false });
    if (error) throw error;
...
async function getDbTeamBySlug(slug) {
  try {
    if (!await isConnected()) {
      return FALLBACK_TEAMS.find((t) => t.id === slug) || null;
    }
    const { data: rows, error } = await supabase.from("teams").select("*").eq("slug", slug);
    if (error) throw error;
    if (!rows || rows.length === 0) return null;
    const row = rows[0];
    return {
...
async function saveDbTeam(data) {
  const conn = await isConnected();
  if (!conn) throw new Error("Database not connected. Cannot perform write operations.");
  const payload = {
    slug: data.slug,
...
async function deleteDbTeam(id) {
  const conn = await isConnected();
  if (!conn) throw new Error("Database not connected. Cannot perform write operations.");
  const { error } = await supabase.from("teams").delete().eq("id", id);
  if (error) throw error;
}
async function getRawTeamsTable() {
  if (!await isConnected()) return [];
  const { data, error } = await supabase.from("teams").select("*").order("pubDate", { ascending: false });
  if (error) throw error;
  return data || [];
}
async function getRawTeamById(id) {
  if (!await isConnected()) return null;
  const { data: rows, error } = await supabase.from("teams").select("*").eq("id", id);
  if (error) throw error;
  return rows && rows.length > 0 ? rows[0] : null;
}

export { getRawBlogsTable as a, getRawServiceById as b, saveDbService as c, deleteDbBlog as d, deleteDbService as e, getRawServicesTable as f, getRawBlogById as g, getRawTeamById as h, saveDbTeam as i, deleteDbTeam as j, getRawTeamsTable as k, getDbBlogs as l, getDbBlogBySlug as m, getDbBlogsExcept as n, getDbServices as o, getDbServiceBySlug as p, getDbTeams as q, getDbTeamBySlug as r, saveDbBlog as s };
